...
class Extractor:
    ...
    def extract_data(self, text: str) -> dict:
        ...
        # 2. Try to find reps
        # Look for explicit reps first, with longest Spanish words first to avoid
        # partial matches
        reps_match = re.search(
            r"(\d+)\s*(repeticiones?|repetitions?|reps?)", text_lower
        )
        if reps_match:
            reps = int(reps_match.group(1))
            text_lower = text_lower.replace(reps_match.group(0), "")
        else:
            # Fallback: If no explicit 'reps' word is used, assume the first number we
            # find is the reps (e.g. "10 sentadillas")
